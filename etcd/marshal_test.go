...
// TestFullDecodeTypedConfig tests the complete encode/decode cycle with typed values for CONFIG ONLY
func TestFullDecodeTypedConfig(t *testing.T) {
	testCases := []struct {
		input         string
		expectedType  string
		expectedValue any
		...
		{"int:42", "int64", int64(42), "integer config decoding"},
		{"float:3.14159", "float64", float64(3.14159), "float config decoding"},
		{"bool:true", "bool", bool(true), "boolean true config decoding"},
		{"bool:false", "bool", bool(false), "boolean false config decoding"},
		{"hello world", "string", "hello world", "string config decoding"},
	}

	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			// First encode as regular string
			encoded, err := encode(tc.input)
			if err != nil {
				t.Fatalf("Failed to encode %s: %v", tc.input, err)
			}

			// Then decode using decodeConfig (with type conversion)
			decoded, err := decodeConfig([]byte(encoded))
			if err != nil {
				t.Fatalf("Failed to decodeConfig %s: %v", encoded, err)
			}

			// Check value
			if decoded != tc.expectedValue {
				t.Errorf("Expected %v, got %v", tc.expectedValue, decoded)
			}
			...
			case "int64":
				if _, ok := decoded.(int64); !ok {
					t.Errorf("Expected int64, got %T", decoded)
				}
			case "float64":
				if _, ok := decoded.(float64); !ok {
					t.Errorf("Expected float64, got %T", decoded)
				}
			case "string":
			...
			case "bool":
				if _, ok := decoded.(bool); !ok {
					t.Errorf("Expected bool, got %T", decoded)
				}
			}
		})
	}
...
// TestGeneralEncodeNoTypeConversion tests that general encode doesn't do type conversion
func TestGeneralEncodeNoTypeConversion(t *testing.T) {
	testCases := []string{
		"int:42",      // Should remain as string
		"float:3.14",  // Should remain as string
		"hello world", // Should remain as string
	}

	for _, input := range testCases {
		t.Run(input, func(t *testing.T) {
			// Encode using general encode (NO type conversion)
			encoded, err := encode(input)
			if err != nil {
				t.Fatalf("Failed to encode %s: %v", input, err)
			}
			...
			// Should remain as string (no type conversion)
			...
			if decodedStr != input {
				t.Errorf("Expected %s, got %s", input, decodedStr)
			}
		})
	}
}
